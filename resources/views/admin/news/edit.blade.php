...
<div style='padding-right:20px;padding-left: 20px;margin-bottom: 20px;'>
    <form method="post" action="{{ route('new.update',$new->id) }}" enctype="multipart/form-data">
        ...
        <div class="form-row">
            ...
            <div class="form-group col-md-6">
                <label for="prenom" class="text-gray-800">Titre de la publication</label>
                <input type="text" class="form-control" value="{{ $new->title }}" name="title" id="inputPassword4">
                @error('title')
                <span class="text-danger">{{ $message }}</span>
                @enderror
            </div>
        ...
        <div class="form-row">
            <div class="form-group col-md-6">
                <label for="age" class="text-gray-800">Photo</label>
        ...
        <div class="form-group ">
            ...
            <textarea class="form-control" id="summernote" name="description" cols="30" rows="10">{{ $new->description }}</textarea>
            @error('description')
            <span class="text-danger">{{ $message }}</span>
            @enderror
        </div>
