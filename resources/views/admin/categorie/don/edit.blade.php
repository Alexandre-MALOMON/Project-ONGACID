<form action="{{ route('categorydon.update',$categoryDon->id)}}" method="POST">
    @csrf
    @method('put')
    <div class="modal fade" id="modalEdit{{$categoryDon->id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" value="{{ GoogleTranslate::trans($categoryDon->name, app()->getLocale()) }}" name="name" id="inputEmail4" >
                        @error('name')
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control" id="editor" name="description" cols="30" rows="10">{{ GoogleTranslate::trans($categoryDon->description ? $categoryDon->description : "Pas de descirption", app()->getLocale()) }}</textarea>
                        @error('description')
                        <span class="text-danger">{{ GoogleTranslate::trans($message, app()->getLocale()) }}</span>
                        @enderror
                    </div>
