...
<div class="form_alert" style="margin-top: 70;">
    ...
    @endif
    <form method="post" action="{{ route('candidature',$emploie->id)}}" enctype="multipart/form-data">
        @csrf
        ...
        <div class="input_alert">
            <label for="">{{ GoogleTranslate::trans('Lettre', app()->getLocale()) }} <span style="color: red;">*</span></label>
            <input type="file" name="lettre"><br>
            @error('lettre')
            <span class="text-danger">{{ GoogleTranslate::trans($message, app()->getLocale()) }}</span>
            @enderror
        </div>
        <div class="input_alert">
            <label for="">{{ GoogleTranslate::trans('Diplome', app()->getLocale()) }} <span style="color: red;">*</span></label>
