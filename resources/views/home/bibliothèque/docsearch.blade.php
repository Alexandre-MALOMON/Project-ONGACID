...
<section class="container_document">
    ...
    <div class="document">
        ...
        <div class="document_text">
            <h2>{{ GoogleTranslate::trans($book->title, app()->getLocale()) }}</h2>
            <h3>{{ $book->auteur}}</h3>
            <p>{{ GoogleTranslate::trans(substr($book->description,0,200), app()->getLocale()) }}...
            </p>

            <div class="container_button_text">
                <div class="link">
                    <a href="{{ route('download',$book->id)}}">{{ GoogleTranslate::trans("TELECHARGER", app()->getLocale()) }}</a>
                </div>
